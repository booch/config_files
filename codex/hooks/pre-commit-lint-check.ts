...
// PreToolUse(exec_command) gate: blocks a `git commit` when a staged file has MORE
// lint issues than the same file at HEAD (a regression). Pre-existing issues never
// block — only net-new ones — so this is safe to run over a repo carrying lint debt.
//
